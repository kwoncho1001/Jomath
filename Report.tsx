...
export const Report: React.FC<ReportProps> = ({ 
    ...
    useEffect(() => {
        const fetchLocalSummary = async () => {
            ...
            if (summary.startsWith("AI 서비스 이용량 초과") || summary.startsWith("AI 기반 피드백 생성에 실패")) {
                ...
                setLocalAiSummaryText(summary); // Fix: Corrected typo from `setLocalSummaryText` to `setLocalAiSummaryText`
            }
    ...
    const handleDownloadPdf = async () => {
        ...
        // 1. 캡처를 위해 실제 화면의 스타일을 일시적으로 고정하는 CSS 주입
        ...
        style.innerHTML = `
            .capturing * {
                ...
                filter: none !important; /* Also remove filters */
                background-image: none !important; /* Force remove gradients for canvas */
            }
            .capturing .h-full {
                /* 컬러바가 0%로 찍히는 것을 방지하기 위해 강제 설정 */
        ...
        try {
            ...
            for (let i = 0; i < sections.length; i++) {
                ...
                const canvas = await html2canvas(section, {
                    ...
                    useCORS: true,
                    backgroundColor: '#ffffff',
                    windowWidth: 1000, // 웹 화면의 레이아웃을 그대로 유지하기 위한 너비 지정
    ...
    return (
        <div className="mt-6 border-t border-gray-200 pt-8">
            ...
            <style>{`
                ...
                /* PDF-specific styles are now dynamically injected by handleDownloadPdf for capture */
                /* This style block remains for general UI-related styles. */
            `}</style>
        </div>
    );
};